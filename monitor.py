...
import joblib  # for loading local model
...
# Set MLflow tracking URI and experiment from environment
    ...
    for k in ref_metrics:
        if k in cur_metrics:
            if k in ["Accuracy", "Precision", "Recall", "F1_Score"]:
                if ref_metrics[k] - cur_metrics[k] > 0.1:  # 10% degradation threshold
                    degraded.append(k)
